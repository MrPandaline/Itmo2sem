...
public class ModelCSVStoragingManager implements IModelStorageManager {
    ...
    @Override
    public LinkedList<Dragon> readFromStorage(IServerLogger logger) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(path))) {
            ...
            //TODO: заменить вызов IOManager на какой-нибудь serverLogger
        } catch(FileNotFoundException e){
            logger.log("Файл коллекции не найден!\n");
        } catch(IOException e){
            logger.log("Что-то пошло не так... файл коллекции не был считан!\n");
        }
        if (haveIncorrectLines) {
            logger.log("Часть элементов коллекции не были считаны! Они исключены из коллекции.\n");
        }
