...
public class ModelBuilder {
    ...
    public Dragon buildDragon() {
        ...
        ioManager.printMessage("Введите возраст дракона (целое число)\nЗначение должно быть в диапазоне (0; 10^6): ", inQuiteMode);
        long age = ioManager.getValidDigit(Long::parseLong, a -> (a > 0 && a < 1000000 ));
        ioManager.printMessage("Введите описание дракона (можно оставить пустым): ", inQuiteMode);
        String description = ioManager.getRawInput();
        ioManager.printMessage("Выберите тип дракона (можно оставить пусты): \n", inQuiteMode);
        DragonType dragonType = buildEnum(DragonType.FIRE, true);
        ioManager.printMessage("Выберите характер дракона: \n", inQuiteMode);
    ...
    public Location buildLocation() {
        ioManager.printMessage("Введите координату x (дробное число). Значение должно быть больше -589: ", inQuiteMode);
        float x = ioManager.getValidDigit(Float::parseFloat, a -> (a > -589));
        ioManager.printMessage("Введите координату y (дробное число): ", inQuiteMode);
        double y = ioManager.getDigit(Double::parseDouble);
        ioManager.printMessage("Введите координату z (целое число): ", inQuiteMode);
    ...
    public Person buildPerson() {
        ioManager.printMessage("Введите имя человека: ", inQuiteMode);
        String name = ioManager.getRawInput();
        ioManager.printMessage("Введите рост человека в мм (целое число): ", inQuiteMode);
        int height = ioManager.getValidDigit(Integer::parseInt, a -> (a > 0 && a < 3000));
        ioManager.printMessage("Введите цвет глаз человека: ", inQuiteMode);
        Color eyeColor = buildEnum(Color.BLACK, false);
        ioManager.printMessage("Введите цвет волос человека: ", inQuiteMode);
        Color hairColor = buildEnum(Color.BLACK, false);
        ioManager.printMessage("Введите национальность человека: ", inQuiteMode);
        Country nationality = buildEnum(Country.CHINA, false);
        ioManager.printMessage("Введите локацию: \n", inQuiteMode);
    ...
    public Coordinates buildCoordinates() {
        ioManager.printMessage("Введите координату x (дробное число). Значение должно быть больше -589: ", inQuiteMode);
        float x = ioManager.getValidDigit(Float::parseFloat, a -> (a > -589));
        ioManager.printMessage("Введите координату y (целое число): ", inQuiteMode);
    ...
    private <T extends Enum<T>> T buildEnum(T enumClass, boolean canBeNull){
        ...
        for (Enum<?> enumeration : enumClass.getDeclaringClass().getEnumConstants()){
            ioManager.printMessage(enumeration + " ", inQuiteMode);
